...
class StellarThemeValidator {
  ...
  async validateMainConfig() {
    ...
    try {
      ...
      if (config.theme !== 'stellar') {
        this.errors.push(
          `主题设置错误: 期望 'stellar', 实际 '${config.theme}'`
        );
      } else {
  ...
  async validateStellarConfig() {
    ...
    if (!fs.existsSync(configPath)) {
      this.errors.push('Stellar主题配置文件 _config.stellar.yml 不存在');
      return;
    }
    ...
    try {
      ...
      if (
        ...
        config.menubar.items.length > 0
      ) {
        this.info.push(
          `✅ 导航菜单已配置 (${config.menubar.items.length} 个项目)`
        );

        // 检查必要的菜单项
        const menuItems = config.menubar.items.map(
