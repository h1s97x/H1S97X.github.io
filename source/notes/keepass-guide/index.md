...
[KeePass 密码保险箱](https://keepass.info/index.html)

[优秀的密码管理器 keepass - 少数派 (sspai.com)](https://sspai.com/post/44126#!)

[一劳永逸：KeePass 全网最详使用指南 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/39645975)
...
1.录入网站域名以及注册的账户、密码等，下次登录时能够自动填充。（Edge 自带的 Microsoft 密码管理器也能解决）
...
KeePass 是一个免费的开源密码管理器，它可以帮助您管理以安全的方式输入您的密码。您可以将所有密码存储在一个密码数据库，该数据库使用主密钥锁定。所以你只需要记住一个单个主密钥可解锁整个数据库。数据库文件已加密使用目前已知的最佳和最安全的加密算法 （AES-256、ChaCha20 和 Twofish）。 有关详细信息，请参阅[功能](https://keepass.info/features.html)页面。

**它真的免费吗？**
是的，KeePass 是真正免费的，不仅如此：它是开源的（OSI 认证）。 您可以查看其完整的源代码并检查安全性 功能已正确实现。
